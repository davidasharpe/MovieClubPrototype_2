<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        else if (record_exists("SELECT * FROM genres WHERE genres.Name LIKE '%{$name}%'")) {
            $message = "<p class='error-msg'>This genre already exists in the database.</p>";
        }
        // Else, insert into database
        else {
            $insert_query = "INSERT INTO genres (Name) VALUES ('{$name}')";
...
<h1 class="center-block">Add Genre to the Database</h1>
<form name="add-producer" action="<?php htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" id="name" name="name" placeholder="e.g. Thriller" data-validation="required" value="<?php if (isset($name)) { echo $name; } ?>">
    </div>
